...
function local_install_java() {
    ...
    Write-Host "Downloading JDK Temurin $version [$os/$arch] from Adoptium to $DTC_JAVA_HOME/jdk.tar.gz"
    ...
    download_file "$adoptium_java_url" "$DTC_JAVA_HOME/jdk.zip"
    ...
    Expand-Archive -Force -LiteralPath "$DTC_JAVA_HOME\jdk.zip" -DestinationPath "$DTC_JAVA_HOME"
    Remove-Item "$DTC_JAVA_HOME\jdk.zip"
